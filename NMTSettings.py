...
class CNMTSettings():
    def __init__(self):
        dprint(__name__, 1, "init class CNMTSettings")
        self.cfg = None
        self.loadSettings()
    ...
    def getSettingsFile(self):
        return sys.path[0] + sep + "NMTSettings.cfg"
...
if __name__=="__main__":
    NMTSettings = CNMTSettings()
    
    UDID = '007'
    NMTSettings.checkSection(UDID)
    
    option = 'transcodequality'
    print NMTSettings.getSetting(UDID, option)
    
    print "setSetting"
    NMTSettings.setSetting(UDID, option, 'True')  # error - pick default
    print NMTSettings.getSetting(UDID, option)
    NMMTSettings.setSetting(UDID, option, '9')
    print NMTSettings.getSetting(UDID, option)
    
    print "toggleSetting"
    NMTSettings.toggleSetting(UDID, option)
    print NMTSettings.getSetting(UDID, option)
    NMTSettings.toggleSetting(UDID, option)
    print NMTSettings.getSetting(UDID, option)
    NMTSettings.toggleSetting(UDID, option)
