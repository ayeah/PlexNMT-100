...
"""
try:
    import lxml.etree.cElementTree as etree
except ImportError:
    try:
...
"""
storage for PMS addresses and additional information - now per NMT! (replaces global PMS_list)
syntax: PMS[<NMT_UDID>][PMS_UUID][<data>]
...
"""
def declarePMS(NMT_udid, uuid, name, scheme, ip, port, type, owned, token):
    # store PMS information in g_PMS database
    global g_PMS
    if not NMT_udid in g_PMS:
        g_PMS[NMT_udid] = {}
    
    address = ip + ':' + port
    baseURL = scheme+'://'+ip+':'+port
    g_PMS[NMT_udid][uuid] = { 'name': name,
...
def updatePMSProperty(NMT_udid, uuid, tag, value):
    # set property element of PMS by UUID
    if not NMT_udid in g_PMS:
        return ''  # no server known for this NMT
    if not uuid in g_PMS[NMT_udid]:
        return ''  # requested PMS not available
    
    g_PMS[NMT_udid][uuid][tag] = value

def getPMSProperty(NMT_udid, uuid, tag):
    ...
    return g_PMS[NMT_udid][uuid].get(tag, '')

def getPMSFromAddress(NMT_udid, address):
    # find PMS by IP, return UUID
    if not NMT_udid in g_PMS:
        return ''  # no server known for this NMT
    
    for uuid in g_PMS[NMT_udid]:
        if address in g_PMS[NMT_udid][uuid].get('baseURL', None):
            return uuid
    return ''  # IP not found

def getPMSAddress(NMT_udid, uuid):
    ...
    return g_PMS[NMT_udid][uuid]['ip'] + ':' + g_PMS[NMT_udid][uuid]['port']

def getPMSCount(NMT_udid):
    # get count of discovered PMS by UUID
    if not NMT_udid in g_PMS:
        return 0  # no server known for this NMT
    
    return len(g_PMS[NMT_udid])
...
parameters:
    ...
    MyPlexToken
result:
    g_PMS database for NMT_udid
"""
def discoverPMS(NMT_udid, CSettings, MyPlexToken=''):
    global g_PMS
    g_PMS[NMT_udid] = {}
    
    #debug
    #declarePMS(NMT_udid, '2ndServer', '2ndServer', 'http', '192.168.178.22', '32400', 'local', '1', 'token')
    #declarePMS(NMT_udid, 'remoteServer', 'remoteServer', 'http', '127.0.0.1', '1234', 'myplex', '1', 'token')
    #debug
    ...
    if CSettings.getSetting('enable_plexgdm')=='False':
        ...
        if XML==False:
            ...
            declarePMS(NMT_udid, uuid, name, 'http', ip, port, 'local', '', '')
        ...
        for uuid in PMS_list:
            PMS = PMS_list[uuid]
            declarePMS(NMT_udid, PMS['uuid'], PMS['serverName'], 'http', PMS['ip'], PMS['port'], 'local', '', '')
    ...
    if not MyPlexToken=='':
        ...
        if XML==False:
            ...
            for Dir in XML.getiterator('Server'):
                ...
                if not uuid in g_PMS.get(NMT_udid, {}):
                    ...
                    declarePMS(NMT_udid, uuid, name, scheme, ip, port, 'myplex', owned, token)
                else:
                    updatePMSProperty(NMT_udid, uuid, 'accesstoken', token)
                    updatePMSProperty(NMT_udid, uuid, 'owned', owned)
    
    # debug print all servers
    dprint(__name__, 0, "Servers (local+MyPlex): {0}", len(g_PMS[NMT_udid]))
    for uuid in g_PMS[NMT_udid]:
        dprint(__name__, 1, str(g_PMS[NMT_udid][uuid]))
...
parameters:
    host
    path
    options - dict() of PlexNMT-options as received from NMT, None for no std. X-Plex-Args
...
def getXArgsDeviceInfo(options={}):
    ...
    if 'PlexNMTUDID' in options:
            xargs['X-Plex-Client-Identifier'] = options['PlexNMTUDID']  # UDID for MyPlex device identification
    if 'PlexNMTName' in options:
            xargs['X-Plex-Device-Name'] = options['PlexNMTName'] # "friendly" name: NMT-Settings->General->Name.
    xargs['X-Plex-Platform'] = 'iOS'
    xargs['X-Plex-Client-Platform'] = 'iOS'
    if 'NMTFirmwareVersion' in options:
        xargs['X-Plex-Platform-Version'] = options['NMTFirmwareVersion']
    xargs['X-Plex-Product'] = 'PlexNMT'
...
result:
    XML
"""
def getXMLFromMultiplePMS(NMT_udid, path, type, options={}):
    root = etree.Element("MediaConverter")
    root.set('friendlyName', 'localServers')
    
    for uuid in g_PMS.get(NMT_udid, {}):
        PMS = g_PMS[NMT_udid][uuid]
        if PMS['type']==type:
            Server = etree.SubElement(root, 'Server')  # create "Server" node
            Server.set('name',    getPMSProperty(NMT_udid, uuid, 'name'))
            Server.set('address', getPMSProperty(NMT_udid, uuid, 'ip'))
            Server.set('port',    getPMSProperty(NMT_udid, uuid, 'port'))
            
            baseURL = getPMSProperty(NMT_udid, uuid, 'baseURL')
            token = getPMSProperty(NMT_udid, uuid, 'accesstoken')
...
parameters:
    ...
    password
    options - dict() of PlexNMT-options as received from NMT - necessary: PlexNMTUDID
result:
    username
...
parameters:
    path
    AuthToken
    options - dict() of PlexNMT-options as received from NMT
...
def getTranscodeImagePath(key, AuthToken, path, width, height):
    ...
    # This is bogus (note the extra path component) but NMT is stupid when it comes to caching images, it doesn't use querystrings.
