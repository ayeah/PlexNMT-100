...
def getTranslation(language):
    ...
    if language not in g_Translations: 
        filename = os.path.join(sys.path[0], 'assets', 'locales', language, 'plexnt.mo')
        try:
            fp = open(filename, 'rb')
...
def pickLanguage(languages):
    ...
    for lang, quot in languages:
        if os.path.exists(os.path.join(sys.path[0], 'assets', 'locales', lang, 'plexnmt.mo')):
                language = lang
                break
    dprint(__name__, 1, "NMTLanguage: "+language)
    return(language)
