...
#except ImportError:
#    try:
#        import xml.etree.cElementTree as etree  #  on Python 2.5+")
#    except ImportError:
#        try:
#            # Python 2.5
...
g_NMTSettings = None
def setNMTSettings(cfg):
    global g_NMTSettings
    g_NMTSettings = cfg
...
"""
global list of known NMTs - to look up UDID by IP if needed

parameters:
    udid - from options['PlexNMTUDID']
    ip - from client_address btw options['NMTAddress']
"""
g_NMTList = {}

def declareNMT(udid, ip):
    global g_NMTList
    if udid in g_NMTList:
        g_NMTList[udid]['ip'] = ip
    else:
        g_NMTList[udid] = {'ip': ip}

def getNMTFromIP(ip):
    # find NMT by IP, return UDID
    for udid in g_NMTList:
        if ip==g_NMTList[udid].get('ip', None):
            return udid
...
# - translate NMT request and send to PMS
# - receive reply from PMS
# - select XSLT template
# - translate to NMT HTML
"""
def XML_PMS2NMT(PMS_baseURL, path, options):
    ...
    #dprint(__name__, 1, prettyXML(XSLtree))
    transform = etree.XSLT(XSLtree)    #dprint(__name__, 1, "PMS_baseURL = " + PMS_baseURL )
    pmsurl = etree.XSLT.strparam(PMS_baseURL)
    if path.endswith('/'):
        nmtpath = etree.XSLT.strparam(path.rstrip('/'))
    else:
        nmtpath = etree.XSLT.strparam(path)
    if 'PlexNMTback' in options:
        nmtback = etree.XSLT.strparam(options['PlexNMTback'])
    else:
        if path == "/" or path == "":
            nmtback = etree.XSLT.strparam("/")
        else:
...
#    XML_ExpandTree(NMTroot, PMSroot, 'main')
#    XML_ExpandAllAttrib(NMTroot, PMSroot, 'main')
#    del g_CommandCollection
    
    dprint(__name__, 1, "====== generated NMT-HTML ======")
    dprint(__name__, 1, prettyXML(NMThtml))
    dprint(__name__, 1, "====== NMT-HTML finished ======")
...
class CCommandHelper():
    def __init__(self, options, PMSroot, PMS_baseURL, path):
        ...
        self.NMT_udid = self.options['PlexNMTUDID']
        self.variables = {}
    ...
    def getKey(self, src, srcXML, param):
        ...
        while '/' in attrib and el!=None:
            ...
            if parts[0].startswith('#'):  # internal variable in path
                ...
                el = el.find(g_NMTSettings.getSetting(UDID, parts[0][1:]))
        ...
        if attrib.startswith('#'):  # internal variable
            ...
            UDID = self.options['PlexNMTUDID']
            res = g_NMTSettings.getSetting(UDID, attrib[1:])
            dfltd = False
...
class CCommandCollection(CCommandHelper):
    ...
    def ATTRIB_SETTING(self, src, srcXML, param):
        ...
        UDID = self.options['PlexNMTUDID']
        return g_NMTSettings.getSetting(UDID, opt)
    ...
    def ATTRIB_URL(self, src, srcXML, param):
        ...
        res = g_param['baseURL']  # base address to PlexNMT
        
        if key.endswith('.js'):  # link to PlexNMT owned .js stuff
            res = res + key
    ...
    def ATTRIB_MEDIAURL(self, src, srcXML, param):
        ...
        if Media!=None:
            # transcoder action
            transcoderAction = g_NMTSettings.getSetting(UDID, 'transcoderaction')
            ...
            if PlexAPI.getPMSProperty(UDID, PMS_uuid, 'type')=='local':
                qLimits = qLookup[g_NMTSettings.getSetting(UDID, 'transcodequality')]
            else:
                qLimits = qLookup[g_NMTSettings.getSetting(UDID, 'remotebitrate')]
            ...
            if transcoderAction=='DirectPlay' \
               ...
               int(Media.get('bitrate','0')) < int(qLimits[2]):
                ...
                settings = ( g_NMTSettings.getSetting(UDID, 'subtitlesize'), \
                             g_NMTSettings.getSetting(UDID, 'audioboost') )
    ...
    def ATTRIB_getDurationString(self, src, srcXML, param):
        ...
        UDID = self.options['PlexNMTUDID']
        if g_NMTSettings.getSetting(UDID, 'durationformat') == 'Minutes':
...
#    param['HostToIntercept'] = 'trailers.apple.com'
    ...
    NMTroot = etree.fromstring(_XML)
    NMTTree = etree.ElementTree(NMTroot)
    print prettyXML(NMTTree)
    
    print
    print "unpack PlexCNMT COPY/CUT commands"
    options = {}
    ...
    print
    print "resulting NMT XML"
    print prettyXML(NMTTree)
    
    print
    #print "store NMT XML"
    #str = prettyXML(aTVTree)
    #f=open(sys.path[0]+'/XML/NMT_fromTmpl.xml', 'w')
    #f.write(str)
    #f.close()
